import React, { useState, useRef, useEffect } from "react";
import { useFormik, Formik } from "formik";
import { Button } from "primereact/button";
import { Dialog } from "primereact/dialog";
import { Toast } from "primereact/toast";
import submit from "@components/form/submitInstallment";
import Field from "@components/form/Field";
import { checkMemberId } from "./NewMemberIntialAPIs";

import { intialFields } from "./VerifyMemberInitialFields";
import { NewMemberIntialFields } from "./NewMemberIntialFields";
import { useRouter } from "next/navigation";
const VeifyInstallmentForm = ({
  newDialog,
  toggleNewDialog,
  handleReload,
  title,
  values,
  hidden,
  disabled,
  required,
  comboOptions,
}) => {
  const router = useRouter();
  const [fields, setFields] = useState([]);
  const [fieldsValues, setFieldsValues] = useState({});
  const [installmentMemberCode, setInstallmentMemberCode] = useState("");
  const [installmentMemberData, setInstallmentMemberData] = useState([]);
  const [memberCode, setMemberCode] = useState(0);
  const [shouldDisplayToast, setShouldDisplayToast] = useState(false);
  const [showToast, setShowToast] = useState(false);
  const [showNext, setNext] = useState(false);
  const [showSave, setSave] = useState(false);
  const [user_id, setUser_id] = useState("");

  useEffect(() => {
    setUser_id(localStorage.clubUser && JSON.parse(localStorage.clubUser).id);
  }, []);

  useEffect(() => {
    if (showNext) {
      setSave(true);
      if (values) {
        Object.entries(values).forEach((val) => {
          if (val[1] === null) values[val[0]] = "";
          formik.setFieldValue(val[0], val[1]);
        });
        setFieldsValues(values);
      }
      hidden &&
        Object.entries(hidden).forEach(
          (val) =>
            (NewMemberIntialFields.find((el) => el.id == val[0]).hidden =
              val[1])
        );
      disabled &&
        Object.entries(disabled).forEach(
          (val) =>
            (NewMemberIntialFields.find((el) => el.id == val[0]).disabled =
              val[1])
        );
      required &&
        Object.entries(required).forEach(
          (val) =>
            (NewMemberIntialFields.find((el) => el.id == val[0]).required =
              val[1])
        );
      comboOptions &&
        Object.entries(comboOptions).forEach(
          (val) =>
            (NewMemberIntialFields.find((el) => el.id == val[0]).comboOptions =
              val[1])
        );
      //only use member_code field and not show all fields until the data is sent
      setFields(NewMemberIntialFields);
    } else {
      if (values) {
        Object.entries(values).forEach((val) => {
          if (val[1] === null) values[val[0]] = "";
          formik.setFieldValue(val[0], val[1]);
        });
        setFieldsValues(values);
      }
      hidden &&
        Object.entries(hidden).forEach(
          (val) => (intialFields.find((el) => el.id == val[0]).hidden = val[1])
        );
      disabled &&
        Object.entries(disabled).forEach(
          (val) =>
            (intialFields.find((el) => el.id == val[0]).disabled = val[1])
        );
      required &&
        Object.entries(required).forEach(
          (val) =>
            (intialFields.find((el) => el.id == val[0]).required = val[1])
        );
      comboOptions &&
        Object.entries(comboOptions).forEach(
          (val) =>
            (intialFields.find((el) => el.id == val[0]).comboOptions = val[1])
        );
      //only use member_code field and not show all fields until the data is sent
      setFields([intialFields[0]]);
    }
  }, [showNext]);

  useEffect(() => {
    // Check if showToast is true and it's not the first render
    if (showToast && shouldDisplayToast) {
      toast.current.show({
        severity: "error",
        summary: "خطأ",
        detail: "هذا العضو غير مشترك بنظام التقسيط",
        life: 3000,
      });
    }
    // Reset shouldDisplayToast
    setShouldDisplayToast(false);
  }, [showToast, shouldDisplayToast]);

  const [loading, setLoading] = useState(false);
  //here will call member code check
  const handleFileChange = async (event) => {
    if (event.target.id == "member_code") {
      setInstallmentMemberCode(event.target.value);
    }
    formik.handleChange(event);
  };

  const handleInstallmentCheck = async (installmentMemberCode) => {
    const installmentData = await checkMemberId(installmentMemberCode);
    if (installmentData?.message === "success") {
      setInstallmentMemberData(Object.values(installmentData.rows[0]));
      setMemberCode(installmentMemberCode);
      setFields(intialFields);
      setShowToast(false);
      console.log(showToast);
    } else {
      setShowToast(true);
      setShouldDisplayToast(true);
      console.log(showToast);
    }
  };

  const handleNext = async () => {
    setNext(true);
  };

  const toast = useRef(null);
  function toggle() {
    formik.handleReset;
    formik.resetForm();
    setFields([intialFields[0]]);
    toggleNewDialog();
  }

  const formik = useFormik({
    initialValues: fields.reduce((obj, field) => {
      if (field.dataType == "text") obj[field.id] = "";
      if (field.value) obj[field.id] = field.value;

      return obj;
    }, {}),

    validate: (data) => {
      let errors = {};
      fields.forEach((field) => {
        if (!data[field.id] && field.required) {
          errors[field.id] = `هذا الحقل إجباري`;
        }
        if (
          field.dataType == "number" &&
          isNaN(data[field.id]) &&
          field.required
        ) {
          errors[field.id] = `هذا الحقل إجباري`;
        }
      });
      return errors;
    },

    onSubmit: async (data) => {
      const utils = {
        setLoading,
        toast,
        toggle,
        handleReload,
        router,
        setSave,
      };
      submit(data, utils);
    },
  });

  const newDialogFooter = (
    <div style={{ textAlign: "left" }} className="mt-5">
      {fields.length > 1 && !showSave ? (
        <Button
          label="التالي"
          icon="pi pi-check"
          iconPos="right"
          loading={loading}
          onClick={() => {
            handleNext();
          }}
        />
      ) : (
        <Button
          label="تحقق"
          icon="pi pi-check"
          iconPos="right"
          disabled={showSave}
          loading={loading}
          onClick={() => {
            handleInstallmentCheck(installmentMemberCode);
            if (!formik.isValid) {
              toast.current.show({
                severity: "error",
                summary: "خطأ",
                detail: "برجاء ملئ/مراجعة الحقول المطلوبة",
                life: 3000,
              });
            }
          }}
        />
      )}
      <Button
        label="حفظ"
        icon="pi pi-check"
        iconPos="right"
        disabled={!showSave}
        loading={loading}
        onClick={() => {
          if (!formik.isValid) {
            toast.current.show({
              severity: "error",
              summary: "خطأ",
              detail: "برجاء ملئ/مراجعة الحقول المطلوبة",
              life: 3000,
            });
          }
          formik.handleSubmit();
        }}
        type="submit"
      />

      <Button
        label="إلغاء"
        iconPos="right"
        icon="pi pi-times"
        onClick={toggle}
      />
    </div>
  );

  const isFormFieldInvalid = (id) => {
    return formik.errors[id];
  };

  const getFormErrorMessage = (name) => {
    return isFormFieldInvalid(name) ? (
      <small className="p-error">{formik.errors[name]}</small>
    ) : (
      <small className="p-error">&nbsp;</small>
    );
  };

  return (
    <>
      <Toast ref={toast} />
      <Dialog
        visible={newDialog}
        style={{ width: "32rem" }}
        breakpoints={{ "960px": "75vw", "641px": "90vw" }}
        header={title}
        headerStyle={{ direction: "rtl", backgroundColor: "#34797F" }}
        rtl={true}
        contentStyle={{ direction: "rtl" }}
        modal
        footer={newDialogFooter}
        onHide={toggle}
      >
        <div className="card flex justify-content-center"></div>
        <div className="card flex justify-content-center">
          <form className="flex flex-column gap-2">
            <br />
            {fields.length > 1 && !showSave
              ? fields.slice(1).map((field, i) => {
                  return (
                    //qual
                    <Field
                      key={field.id}
                      id={field.id}
                      name={field.name}
                      isFormFieldInvalid={isFormFieldInvalid}
                      getFormErrorMessage={getFormErrorMessage}
                      onChange={handleFileChange}
                      value={
                        installmentMemberData.length > 0
                          ? installmentMemberData[i]
                          : "العضو غير مشترك"
                      }
                      fieldType={field.fieldType}
                      lookup={field.lookup}
                      dataType={field.dataType}
                      required={field.required}
                      disabled={field.disabled}
                      accept={field.accept}
                      hidden={field.hidden}
                      compound={field.compound}
                      comboOptions={field.comboOptions}
                      // apiData={installmentMemberData}
                    />
                  );
                })
              : fields.map((field) => {
                  return (
                    //qual
                    <Field
                      key={field.id}
                      id={field.id}
                      name={field.name}
                      isFormFieldInvalid={isFormFieldInvalid}
                      getFormErrorMessage={getFormErrorMessage}
                      onChange={handleFileChange}
                      value={fieldsValues[field.id]}
                      fieldType={field.fieldType}
                      lookup={field.lookup}
                      dataType={field.dataType}
                      required={field.required}
                      disabled={field.disabled}
                      accept={field.accept}
                      hidden={field.hidden}
                      compound={field.compound}
                      comboOptions={field.comboOptions}
                    />
                  );
                })}
          </form>
        </div>
      </Dialog>
    </>
  );
};

export default VeifyInstallmentForm;
